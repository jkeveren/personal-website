...
export default ({targetHostname, targetURL}) => {
	...
	<!--
		...
		Anyway heres a few unnecessarily cryptographically random and HTML-invalid bytes (generated at server start):
	...
	const writeText = async (res, text) => {
	...
	const createSlowTextWriter = res => async text => {
	...
	const createLinkWriter = (res, writeSlowText) => async (url, text) => {
	...
	return async ({res}) => {
		const writeSlowText = createSlowTextWriter(res);
		const writeLink = createLinkWriter(res, writeSlowText);
...
// 		]) {
// 			await writeSlowText('- ');
// 			res.write(`<a target=_blank href=${url}>`);
// 			await writeSlowText(text);
// 			res.write('</a><br>');
// 		}
